...
import AdminHeaderBs from '../../../header/AdminHeaderBs';
...
function BasicScience() {
  ...
  const createBranch = async () => {
    const { data, error } = await supabase
      .from('branches')
      .insert([{ branch_name: branchName, program_id: 3 }])
      .select();
  ...
  const fetchDetails = async () => {
    try {
      let { data, error } = await supabase.rpc('get_all_basicsci_programs_info');
      if (error) {
  ...
  return (
    ...
    <div className="p-6 text-center">
      ...
      {showForm && (
        <div className="bg-gray-800 p-6 rounded-md w-full max-w-md mx-auto mt-6 text-white">
      ...
      <div className="space-y-4">
        {Array.isArray(courses) && courses.length > 0 ? (
          courses.map((course, index) => (
            <div key={index} className="bg-gray-800 max-w-md p-4 rounded-md text-slate-100">
