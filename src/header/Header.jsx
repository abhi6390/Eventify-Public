...
const menuItems = [
  ...
  { name: "Open Elective", href: "/student/openelective" },
  { name: "Telegram Bot", href: "/student/telegrambot" },
    ...
    <div ref={headerRef} className="sticky top-0 z-10 w-full bg-blue-500">
      ...
      {isSidebarOpen && (
        <aside
          ref={sidebarRef}
          style={{
            top: `${headerHeight}px`, // Start the sidebar just below the header
            height: `calc(100% - ${headerHeight}px)`, // Make the sidebar fill the remaining height
          }}
          className="fixed left-0 z-20 bg-blue-500 opacity-95 w-[50%] sm:w-1/2"
        >
          {/* Sidebar Links */}
          <nav className="mt-6 px-6 flex flex-col space-y-3">
            {menuItems.map((item) => (
              <Link
                to={item.href}
                key={item.name}
                onClick={toggleSidebar}
                className="block py-3 text-white text-lg font-semibold hover:bg-gray-600 rounded-md"
              >
                {item.name}
              </Link>
            ))}
